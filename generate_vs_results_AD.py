...
USAGE    %s  [-f filename|-d directory] -r receptor [ OPTIONS ]
            ...
            -F filename     file containing fullpath list of ligands to process
...
OPTIONS
            ...
            -x filename     calculate and print RMSD by using this reference PDBQT structure
                            (heavy atoms only)
            -W filename     use the map 'filename' to filter and score hydrated dockings
...
if __name__ == '__main__':
    ...
    try:
        options, var = getopt.getopt(argv[1:], 'f:d:F:r:o:t:ivm:Al:RAh:x:W:Eq')
    ...
    if '-f' in opts:
        dlg_list = opts['-f'].split(",")
        if DEBUG: print "[ file mode:", input_file,"]"
        ...
        if recursive: print "- scanning sub-directories of [ %s ]..." % dir_root
        ...
        fname = opts['-F']
        if DEBUG: print "[ filelist mode: %s ]" % fname
        try:
            dlg_list = getLines(fname, doStrip=1)
            print "- %d DLG found in file %s" % (len(dlg_list), fname)
        except:
            print "ERROR: impossible to read the ligand list from file [%s]" % fname
            exit(2)
    else:
    ...
    # end parsing options
    if not len(dlg_list):
        print "No input files found... Exit." 
        exit(0)

    print "- scanning for ligand names...",
    input_file, problematic = findLigandsInDlg(dlg_list, checkSuccess = checkSuccess)
    print "\r-"
    print " [ %d ligands, %d skipped/problematic ]" % (len(input_file), len(problematic))
    ...
    if len(problematic):
        print "[ =>  %d problematic result files are saved in log file : %s ]" % ( len(problematic), probl_log)
        fp = open(probl_log, 'w')
    ...
    if receptor:
        ...
        recname = ""
    tot = len(input_file.keys())
    for l in input_file.keys():
        #mode = requested_mode
        #print "================= [ %d ]"% len(input_file[l])
        ...
        try:  # XXX CHANGE THIS IN THE FINAL VERSION!!
            ...
            for p in pool:
                ...
                out_line = "%s\t%d\t%2.3f\t%2.3f\t%d\t%2.1f\t%d\t%s\t%s%s\n" % ( processor.ligName, p+1, pose['energy'],
                           pose['leff'], pose['csize'], pose['cpercent'], len(processor.poses), recname, output_filename, hydrated)

                result_sort.append( (processor.ligName, p+1,  pose['energy'], pose['leff'], pose['csize'],
                            pose['cpercent'], len(processor.poses), recname, output_filename, hydrated) )

                if '-l' in opts:
                    log.write(out_line)
                    if not QUIET: print "\r- processing ... %2.2f%% [ %d | %d  ]                     " % (percent(c, tot), c, tot),
                else:
                    if not QUIET:
                        print out_line[:-1]
                if DEBUG:
    ...
    if '-l' in opts:
        ...
        for r in result_sort:
            if len(r) < 10:
                r.append("")
            out_line = "%s\t%d\t%2.3f\t%2.3f\t%d\t%2.1f\t%d\t%s\t%s\t%s" % ( tuple(r) )
            fp.write(out_line+'\n')
        fp.close()
